...
const ApplicationDetails = (props) => {
  ...
  const submitAction = async (data, nocData = false, isOBPS = {}) => {
    if(data?.Property?.workflow?.comment?.length == 0 || data?.Licenses?.[0]?.comment?.length == 0 || data?.WaterConnection?.comment?.length == 0 || data?.SewerageConnection?.comment?.length == 0)
    {
